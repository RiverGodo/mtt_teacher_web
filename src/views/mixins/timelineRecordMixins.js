...
export default{
    ...
    methods: {
        getTreats(d){
            postData('/v1/organization/getOrgCaseDetail',{case_id: d || +this.$route.query.case_id}).then((res) => {
                if(res.res_code==1) this.treats = this.$config.setDataK(res.data.treatment)
            })
        },
        getObjectivetests(d){
        ...
        getMockDiagnoses(d){
            postData('/v1/organization/getOrgCaseDetail',{case_id: d || +this.$route.query.case_id}).then(async(res) => {
                if(res.res_code==1) {
                    this.mockDiagnoses = res.data.diagnose_list;
                    this.mockDiagnosesInit = this.$config.copy(res.data.diagnose_list,[])
                    let asyncWorker = (t,i) => {
