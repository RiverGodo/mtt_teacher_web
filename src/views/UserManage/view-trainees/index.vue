<template>
   <div class="case">  
        <ranking v-if="isRank" :hInfo="hInfo" :isShow="rankingShow" @close="rankingClose" :title="rankingTitle" :type="rankingType"/>
        ...
        <div class="case-main" :style="list.length==0 ? 'height: calc(100% - 170px);' : activeItem ? 'height: calc(100% - 240px);' : 'height: calc(100% - 160px);'">
            <div v-if="activeItem&&hInfo" class="statistics-view">
                ...
                <span v-if="activeItem == 1" class="s-item">班级排名：{{hInfo.in_grade_sort_num}}</span>
                <span v-if="activeItem == 2" class="s-item">综合排名：{{hInfo.in_grade_sort_num}}</span>
            ...
            <div class="content" :style="activeItem ? 'height: calc(100% - 40px);' : 'height: calc(100% - 30px);'">
                ...
                <div class="content-main" :style="activeItem ? 'padding-top: 30px;' : 'margin-top: 5px'">
                    ...
                    <BrokenLine type="abilityAnalysis1" id="abilityAnalysis1" title="考试成绩趋势" url="/v1/user/getExamChart" v-if="activeItem == 0" />
                    <BrokenLine type="abilityAnalysis2" id="abilityAnalysis2" title="学习时长（分钟）" url="/v1/user/getStudyChart" v-if="activeItem == 0" />
...
export default {
    ...
    data(){
        return{
            ...
            taskList: [],
            menuList: [
                { img: 'ability', title: '综合分析', active: true},
    ...
    methods: {
        getEcherts(){
            postData('/v1/user/getSystemChart', {user_id: +this.$route.query.id}).then((r) => {
                if(r.res_code == 1) { 
                    this.setEcherts1(r.data.sysAbi)
                    this.valueCustom = +r.data.verb
                }
            })
        },
        getEcherts2(){
            postData('/v1/user/getDiagnoseChart', {user_id: +this.$route.query.id}).then((r) => {
                if(r.res_code == 1) { 
                    this.setEcherts2(r.data.dtAbi)
                    this.valueCustom2 = +r.data.verb
                }
            })
        },
        rankingClose(){
        ...
        getHeadData(){
            ...
            //     }
            // })
            this.patientData =  JSON.parse(sessionStorage.getItem('STUDENRTABLEROWDATA'))
        },
        getExerciseBaseInfo(url, field){
            postData(url, {user_id: +this.$route.query.id,grade_id: +this.$route.query.grade_id,id:+this.$route.query.grade_id,homework_id:+this.$route.query.grade_id,exam_id:+this.$route.query.grade_id}).then(r => {
            ...
            postData(url,d).then((res) => {
                if(res.res_code==1) {
                    if(res.data&&res.data.length>0){
                        let d = this.$config.setDataInit(res.data.list)
                        this[field] = d
                    }
                }
            })
    ...
    created(){
        new Promise((resolve,reject) => {
            ...
            this.getList('/v1/exercise/getExerciseList','practiceList')
            this.getExerciseBaseInfo('/v1/exercise/getExerciseInfo', 'practiceInfo')
            return true
        }).then(() => {
            this.getList('/v1/exam/getStudentExamList','examList')
            this.getExerciseBaseInfo('/v1/exam/getStudentExamInfo', 'examInfo')
            return true
        }).then(() => {
            this.getList('/v1/homework/getHomeworkListByStudent','taskList')
            this.getExerciseBaseInfo('/v1/homework/getStudentHomeworkInfo', 'taskInfo')
            return true
        })
    }
}
...
@media screen and (max-height: 870px){
    .case .case-main .content .content-main .no-time{
        margin-top: 20px;
    }
}
</style>
