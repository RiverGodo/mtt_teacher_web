   ...
   <div class="case">
        ...
        <div class="case-main">
            ...
            <Tabs v-model="activePane" @on-click="tabs">
                ...
                <TabPane v-if="type == 1" label="病例提纲" name="name3" class="panes2">
                    <outlines ref="ViewReportsOutline" />
                </TabPane>
            </Tabs>
        </div>
...
export default {
    components: { see, DiaryTreat, ScoringModal, outlines },
    ...
    computed:{
        ...
        scores(){
            ...
            if(this.type == 2) {
                ...
                let op = this.caseInfo&&this.caseInfo.operator ? '操作人：'+`<span style=${operator}>${this.caseInfo.operator}</span>` : ''
    ...
    methods: {
        setOutlinesData(){
            PostData('case/getCaseDetailAndOutlines', {id: +this.$route.query.case_id}).then(res => {
                if(res.res_code == 1) {
                    this.$refs.ViewReportsOutline.data = JSON.parse(res.data.outlines[0].outline)
                    this.$refs.ViewReportsOutline.graph.read(JSON.parse(res.data.outlines[0].outline))
                }
            })
        },
        confirmFraction(){
        ...
        getExamHeadData(url,d){
            ...
            postData(url, d).then((res) => {
                if(res.res_code==1) {
                    ...
                    this.patientData.title = res.data.exam_name || res.data.homework_name
        ...
        tabs(name){
            if(name == 'name3') this.setOutlinesData()
            else{
