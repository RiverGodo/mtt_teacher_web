        ...
        methods: {
            setCustom(){
                ...
                if(this.times == 'CUSTOM'&&this.data_picker.length==2){
                    ...
                    d.start_time = moment(new Date()).subtract(1,'months').format('YYYY-MM-DD HH:mm:ss')
                    d.end_time = moment(new Date()).format('YYYY-MM-DD HH:mm:ss')
                }else if(this.times == '6MONTH'){
                    d.start_time = moment(new Date()).subtract(6,'months').format('YYYY-MM-DD HH:mm:ss')
                    d.end_time = moment(new Date()).format('YYYY-MM-DD HH:mm:ss')
                }else if(this.times == 'YEAR'){
                    d.start_time = moment(new Date()).subtract(1,'year').format('YYYY-MM-DD HH:mm:ss')
                    d.end_time = moment(new Date()).format('YYYY-MM-DD HH:mm:ss')
                }
                return d
            },
            dataPickerChange(val) {
                ...
                if(this.data_picker.length==2&&this.data_picker[0]) this.getEcharts()
            },
            getEcharts(){
                let d = {
            ...
            setOptions(d, dx){
                let obj = {
                    abilityAnalysis1: [{name: '本人成绩', icon: 'circle'},{name: '平均成绩', icon: 'circle'}],
                    abilityAnalysis2: []
                }
                let setData = (name, d) => {
                    return  {
                ...
                return {
                    ...
                    tooltip: {
                        padding: 15,
                        trigger: 'axis',
                        formatter: this.type == 'abilityAnalysis1' ? `{b}<br/> <div style='float: left'>{a0}：{c0}</div><br/>
                <div style='float: left'>{a1}：{c1}</div><br/>` : `{b}<br/> <div style='float: left'>{a}：{c}</div>`
                    },
                    legend: {
                        data: legendData
                    },
                    ...
                    yAxis: {
                        ...
                        splitLine :{    //网格线
                            lineStyle:{
                                type:'dashed' ,  color: '#D4D7E3'  //设置网格线类型 dotted：虚线   solid:实线
                            },
                            show:true //隐藏或显示
                        },
                        axisLabel: {
            ...
            setEcherts1(d, dx){
                ...
                myChart.setOption(this.setOptions(d, dx), true);
            },
        }
    }
</script>
