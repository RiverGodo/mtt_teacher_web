    ...
    <div style="display: flex; height: 100%">
        ...
        <div style="overflow-x: auto; flex: 1; backgroundColor: #f0f0f7">
            ...
            <div class="screen">
                ...
                <Button style="margin-right: 20px;margin-left: auto;" @click="handleClick()" type="primary">创建体检</Button>
            </div>
            <div class="content-box">
                <!-- <tables :tabel-height="tableHeight" :is-serial="true" :column="columns1" :table-data="list" @operation1="editCase" @operation2="deleteCase" @operation="checkCase"></tables> -->
        ...
        <Modal v-model="checkInquiry" :mask-closable="false" :footer-hide="true" :title="modalTitle" width="700" @on-cancel="cancelCheck">
            <!-- {{checkResult}} -->
            <div v-if="modalTitle == '查看体格检查'">
                <div v-if="checkResult.type == 1">
                    <img :src="url + checkResult.url" alt
                         style="width:100%"/>
                    <!-- <img v-for="(item, index) in checkResult.child" :key="index" :src="url + item.result_media_url" alt
                         style="width:100%"/> -->
                </div>
                <div v-else-if="checkResult.type == 3">
                    <video  class="collapse-video" autoplay
                           controls width="100%"
                           height="100%" :src="url + checkResult.url" ref="checkVideo"></video>
                ...
                <div v-else-if="checkResult.type == 2">
                    <audio  class="collapse-audio"
                           :src="url + checkResult.url"
                ...
                <!-- <div v-if="checkResult.result_media_type == 'None'" style="fontSize: 16px">{{checkResult.result_text}}</div> -->
                <div>{{checkResult.result_text}}</div>
            ...
            <div v-else>
                <div class="add-physical">
                                        ...
                                        <Dropdown trigger="click" @on-visible-change="handleShowTree(item,index)" placement="right-start">
                                            ...
                                            <DropdownMenu slot="list" class="drop-menu">
                                                <span v-for="(item1, index1) in item.children" :key="index1">
                                                    <Dropdown  @on-visible-change="handleShowTree2(item1,index,index1)" trigger="click" placement="right-start">
                                                        <DropdownItem class="drop-item">
                    ...
                    <div>
                        ...
                        <Select v-model="body_form.apply_gender" class="inquiry-type">
                    ...
                    <div>
                        ...
                        <Select v-model="body_form.apply_age" class="inquiry-type">
        ...
        <Modal v-model="alterModal" :mask-closable="false" :footer-hide="true" title="体格检查类型管理" width="700" class="alter-modal">
    ...
    export default {
        ...
        data() {
            return {
                ...
                type_data: {},
                body_form: {
                    case_physical_method_id: null,
                    result_text: "",
                    apply_gender: "",
                    apply_age: "",
                    result_attachment_type:null,
                    file_name:null,
                    type:null,
                    url:null
                },
        ...
        methods: {
            handleShowTree(value,index){
                postData('/v1/common/getPhysicalCategory',{parent_id:value.id}).then(res => {
                    this.assist_tree[index].children = res.data
                    this.$forceUpdate()
                })
            },
            handleShowTree2(value,index,index2){
                postData('/v1/physical/getPhysicalMethodList',{category_id:value.id}).then(res => {
                    this.assist_tree[index].children[index2].children = res.data
                    this.$forceUpdate()
                })
            },
            splitFile(name, type) {
            ...
            firstChange(val) {
                if (val.length) {
                    this.second = null;
                    postData("/v1/common/getPhysicalCategory", {parent_id: parseInt(val[0])}).then(
                        res => {
                            if (res.res_code == 1) {
            ...
            secondChange(val) {
                if (val.length) {
                    postData("/v1/physical/getPhysicalMethodList", {
            ...
            deleteCase(val) {
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        postData("/v1/physical/removePhysicalTemplate", {
                            id: val.id
                        }).then(res => {
                            if (res.res_code) {
            ...
            handleCheck(item, index) {
                ...
                let data = {
                    ...
                    apply_age: this.age_state == "all" ? null : this.age_state,
                    apply_gender: this.gender == "all" ? null : this.gender,
                    page_size: this.pageSize,
                    page_num: this.current,
                    case_physical_method_id: this.check_data.id
                };
                postData("/v1/physical/getPhysicalTemplateList", data).then(res => {
                    console.log(res);
                    if (res.res_code == 1) {
            ...
            checkPhysical() {
                let url = this.modalTitle == "创建体格检查" ? "/v1/physical/addPhysicalTemplate" : "case/modifyPhysical";
                this.body_form.case_physical_method_id = this.check_data.id;
            ...
            getList() {
                ...
                postData("/v1/physical/getPhysicalTemplateList", d).then(res => {
                    if (res.res_code == 1) {
                        res.data.data.forEach((element,index) => {
                            ...
                            element.question_and_answer = element.question + element.answer;
                        });
            ...
            getCategory(update) {
                return postData("/v1/common/getPhysicalCategory",{parent_id:0}).then(res => {
                    if (res.res_code == 1) {
            ...
            getMoldTree() {
                postData('/v1/common/getPhysicalCategory',{parent_id:0}).then(res => {
            ...
            append(data) {
                if (!data.level){
                    ...
                    let url = ""
                    url = data.level == 2 ? "/v1/physical/getPhysicalMethodList" : "/v1/common/getPhysicalCategory"
                    postData(url, {parent_id: parseInt(data.id),category_id: parseInt(data.id)}).then(res => {
                        if (res.res_code == 1) {
                            data.children = [];
                            res.data.forEach(item => {
                                data.children.push({
            ...
            remove(root, node, data) {
                ...
                let url = ""
                if (data.level == 3) {
                    url = "/v1/physical/removePhysicalMethod"
                } else {
                    url = "/v1/physical/removePhysicalCategory"
                }
                this.$Modal.confirm({
                    title: "确认要删除这项类型吗",
                    content: "删除后不会对已发布的病例造成影响",
            ...
            modifyInput(val, data, root, node) {
                if (val && data.is_create) {
                    ...
                    let url = ""
                    if (data.level == 3) {
                        url = "/v1/physical/addPhysicalMethod"
                    } else {
                        url = "/v1/physical/addPhysicalCategory"
                    }
                    ...
                    let url = ""
                    if (data.level == 3) {
                        url = "/v1/physical/modifyPhysicalMethod"
                    } else {
                        url = "/v1/physical/modifyPhysicalCategory"
                    }
                    postData(url, {
                        id: data.id,
                        name: val
            ...
            onToggleExpand(val) {
                ...
                data.parent_id = parseInt(val.id) ? parseInt(val.id) : 0
                data.category_id = parseInt(val.id) ? parseInt(val.id) : 0
                let urls = val.level == 2 ? "/v1/physical/getPhysicalMethodList" : "/v1/common/getPhysicalCategory"
                postData(urls, data).then(res => {
                    if (res.res_code == 1) {
                        val.children = [];
                        res.data.forEach(item => {
                            val.children.push({
