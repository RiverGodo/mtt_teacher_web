<template>
    <div style="display: flex; height: 100%">
        <div class="modal-box">
            ...
            <Collapse style="text-align: left; padding-left: 15px" v-model='first' :accordion="true" simple @on-change="firstChange">
                <Panel v-for="(item,index) in firstList" :key="index" :name="String(item.id)">
                    <Tooltip theme="light" :content="item.name">
                    ...
                    <Collapse v-if="item.hasCatelog" style="height: 100%;overflow-y: auto" v-model='second'
                        ...
                        <Panel v-for="(item1,index1) in secondList" :key="index1" :name="String(item1.id)">
                            <Tooltip theme="light" :content="item1.name">
                                <span class="item-head">{{item1.name}}</span>
                            </Tooltip>
                            <div v-for="(it, ind) in thirdList" :key="it.id" slot="content" class="panel-content"
                                 @click="handleCheck(it, ind, 1)" :class="it.checked ? 'panel-content-checked' : ''">
                                <Tooltip theme="light" :content="it.name">
        ...
        <div style="overflow-x: auto; flex: 1; backgroundColor: #f0f0f7">
            ...
            <div class="screen">
                <Select @on-change="selectChange2" class="select-list" v-model="gender">
                    <Option v-for="(item,index) in select2" :value="item.id" :key="index">{{item.title}}</Option>
                </Select>
                <Select @on-change="selectChange2_1" class="select-list" v-model="age_state">
                    <Option v-for="(item,index) in select2_1" :value="item.id" :key="index">{{item.title}}</Option>
                </Select>
                <div class="screen_input1">
                    <Input style="width:300px;margin-left: 10px;" v-model="keyword" @on-change="inputChange()" placeholder="搜索内容" />
                    <Icon type="md-search" slot="prefix"/>
                </div>
                <Button style="margin-right: 20px;margin-left: auto;" @click="handleClick()" type="primary">创建辅检</Button>
            </div>
            <div class="content-box">
                <!-- <tables :tabel-height="tableHeight" :is-serial="true" :column="columns1" :table-data="list" @operation="checkCase" @operation1="editCase" @operation2="deleteCase"></tables> -->
                <Table :height="tableHeight" :columns="columns1" :data="list">
                    <template slot-scope="{ row, index }" slot="operation">
                        <div class="action">
                            <span @click="checkCase(row,index)" class="see">查看</span>
                            <span @click="editCase(row,index)"  class="nosee">编辑</span>
                            <span @click="deleteCase(row,index)" class="nosee">删除</span>
                        </div>
                    </template>
                </Table>
        ...
        <Modal v-model="checkInquiry" :mask-closable="false" :footer-hide="true" :title="modalTitle" width="700" class="check-modal" :styles="{marginBottom: '100px'}" @on-cancel="handleCancel">
            <div v-if="modalTitle == '查看辅助检查'">
                <div v-if="data_type == 'ExaminationReport'">
                    <!-- <tables :tabel-height="580" :column="columns2" :table-data="check_list" @inputChange="inputContentChange"/> -->
                    <Table :columns="columns2" :data="check_list"></Table>
                </div>
                <div v-else-if="data_type == 'ImageData'" style="height: 580px;overflow: hidden">
                    <img v-if="check_list[0].type == 'Picture'" style="width: 100%;" :src="devurl + check_list[0].report" alt="">
                    <video v-else-if="check_list[0].type == 'Video'" :src="devurl + check_list[0].report"></video>
                    <audio v-else-if="check_list[0].type == 'Audio'" :src="devurl + check_list[0].report"></audio>
                    <iframe v-else :frameborder="0" class="case-ifram" :src="officeappsUrl + check_list[0].report"></iframe>
                </div>
                        ...
                        <Dropdown trigger="click">
                            ...
                            <DropdownMenu slot="list" class="drop-menu">
                                <span v-for="(item, index) in assist_tree" :key="index">
                                    <DropdownMenu slot="list" class="drop-menu">
                                        <Dropdown trigger="click"  @on-visible-change="handleShowTree(item,index)" placement="right-start">
        ...
        <Modal v-model="alterModal" :mask-closable="false" :footer-hide="true" title="辅助类型管理" width="700" class="alter-modal">
            <div class="alter-hint">辅助检查类型支持两级菜单</div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableHeight: null,
                keyword: '',
                devurl:"http://sfile.9mededu.com/mtt/Data/main/Cases/",
                mold_head: [
                ...
                },
                {
                    ...
                    minWidth: 120,
                    render: (h, param) => {
                        // let opinion = {0: '通用', 1: '男', 2: '女'}
                        let opinion = ['','男','女','通用'] 
                        return h('span', opinion[param.row.apply_gender] ? opinion[param.row.apply_gender] : '-')
                    }
                }, {
                    title: '适用年龄',
                    key: 'apply_age',
                    ...
                    render: (h, param) => {
                        ...
                        let opinion = ['','婴幼儿','少年','青年','中年','老年','通用']
                        return h('span', opinion[parseInt(param.row.apply_age)])
                    }
                }, {
                    title: '更新时间',
                    key: 'update_time',
        ...
        methods: {
            handleShowTree(value,index){
                postData('/v1/assistant/getAssistantMethodList',{category_id:value.id}).then(res => {
                    this.assist_tree[index].children = res.data
                    this.$forceUpdate()
                })
            },
            splitFile(name) {
            ...
            checkPhysical() {
                ...
                if (this.body_form.id) {
                    ...
                    //文件类型 type 1 图片 2 音频 3 视频
                    //附件类型 result_attachment_type 0 无附件 1 文件 2 表格
                    this.body_form.item_id = this.type_data.id || this.check_data.id
                    let bodydata = {
                        case_assistant_method_id:this.body_form.item_id,
                        apply_gender:this.body_form.gender,
                        apply_age:this.body_form.age_state,
                        result_attachment_type:0
                    }
                    postData('/v1/assistant/addAssistantTemplate', bodydata).then(res => {
                        if (res.res_code == 1) {
                            this.checkInquiry = false
                            this.handleCheck()
                        }
            ...
            firstChange(val) {
                ...
                if (val.length) {
                    postData('/v1/assistant/getAssistantMethodList', {category_id: parseInt(val[0])}).then(res => {
                        if (res.res_code == 1) {
            ...
            checkCase(val) {
                ...
                if (val.type != "None") {
                    ...
                    this.$Message.success(val.result_text)
                }
            },
            hanldSubmit() {
                this.checkInquiry = false
            ...
            editCase(val) {
                ...
                if (this.body_form.result_attachment_type == 0) {
                ...
                this.data_type = this.body_form.result_attachment_type
            ...
            deleteCase(val) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        postData('/v1/assistant/removeAssistantTemplate', {id: val.id}).then(res => {
                            if (res.res_code) {
            ...
            handleCheck(item, index, current) {
                if (item) this.check_data = item
                if(current) this.current = current
                let data = {
                    keyword: this.keyword,
                    age_state: this.age_state == "all" ? null : this.age_state,
                    gender: this.gender == "all" ? null : this.gender,
                    page_size: this.pageSize,
                    page_num: this.current,
                    case_assistant_method_id: this.check_data.id
                }
                postData('/v1/assistant/getAssistantTemplateList', data).then(res => {
                    if (res.res_code == 1) {
                        ...
                        res.data.list.forEach((element,index) => {
                            ...
                            postData('/v1/assistant/getAssistantTemplateDetail',{id:element.id}).then(res2 => {
                                console.log(res2,"wsq");
                                if (element.result_attachment_type == 2) {
                                    element.child = JSON.parse(res2.data.content)
                                }
                                element.type = element.result_attachment_type == 2 ? "ExaminationReport" : element.result_attachment_type == 1 ? "ImageData" : 'None'
                                element.gender = element.apply_gender
                                element.age_state = element.apply_age
                            })
                        });
                        setTimeout(() => {
            ...
            getMoldTree() {
                postData('/v1/common/getAssistantCategory',{parent_id:0}).then(res => {
                    if (res.res_code == 1) this.assist_tree = res.data
                })
            },
            getCategory(update) {
                return postData('/v1/common/getAssistantCategory',{parent_id:0}).then(res => {
                    if (res.res_code == 1) {
            ...
            append(data, type) {
                ...
                let url = ""
                if (data.level == 1) {
                    url = "/v1/assistant/getAssistantMethodList"
                } else {
                    url = "/v1/common/getAssistantCategory"
                }
                ...
                else postData(url, {parent_id: parseInt(data.id ? data.id : 0),category_id: parseInt(data.id ? data.id : 0)}).then(res => {
                    if (res.res_code == 1) {
                        data.children = []
                        res.data.forEach(item => {
                            data.children.push({
            ...
            remove(root, node, data) {
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        let url = ""
                        if (data.level == 2) {
                            url = "/v1/assistant/removeAssistantMethod"
                        } else {
                            url = "/v1/assistant/removeAssistantCategory"
                        }
                        postData(url, {id: data.id, type: data.type}).then(res => {
                            if (res.res_code == 1) {
            ...
            modifyInput(val, data, root, node) {
                if (val && data.is_create) {
                    ...
                    let url = ""
                    if (data.level == 2) {
                        url = "/v1/assistant/addAssistantMethod"
                    } else {
                        url = "/v1/assistant/addAssistantCategory"
                    }
                    let data_obj = {
                        name: val,
                        parent_id,
                    ...
                    postData(url, data_obj).then(res => {
                        if (res.res_code == 1) {
                            this.getCategory(1)
                    ...
                    let url = ""
                    if (data.level == 2) {
                        url = "/v1/assistant/modifyAssistantMethod"
                    } else {
                        url = "/v1/assistant/modifyAssistantCategory"
                    }
                    postData(url, {id: data.id, name: val, type: data.type}).then(res => {
            ...
            onToggleExpand(val) {
                ...
                console.log(val);
                postData(url,{category_id: parseInt(val.id ? val.id : 0),parent_id: parseInt(val.id ? val.id : 0)}).then(res => {
                    val.children = []
                    res.data.forEach(item => {
                        val.children.push({
                            title: item.name,
