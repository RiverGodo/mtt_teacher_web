    ...
    <div style="display: flex; height: 100%">
        ...
        <div style="overflow-x: auto; flex: 1">
            ...
            <div class="screen">
                ...
                <span v-html="hint" style="font-size: 16px;margin-left: 10px;" class="all-content"></span>
                <Button style="margin-right: 20px;margin-left: auto;" @click="handleClick()" type="primary">创建问诊</Button>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editCase(val) {
                [this.editId, this.inquiryType, this.sexType, this.ageType, this.askInput, this.answerInput] = [val.id, val.category_id, val.apply_gender == '-' ? '' : val.apply_gender, val.apply_age,val.question, val.answer]
            ...
            deleteCase(val) {
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        postData('/v1/inquire/removeInquireTemplate', {id:val.id}).then(res => {
                            if(res.res_code){
            ...
            handleSubmit() {
                let data = {
                    ...
                    apply_gender: this.sexType,
                    apply_age: this.ageType,
                }
                if(this.modalTitle == '添加问诊') {
                    postData('/v1/inquire/addInquireTemplate', data).then(res => {
                        if(res.res_code == 1) {
                    ...
                    data.id = this.editId
                    postData('/v1/inquire/modifyInquireTemplate',data).then(res => {
                        if(res.res_code == 1) {
                            this.checkInquiry = false
                            this.$Message.success(res.msg)
                            this.getList()
            ...
            getList() {
                ...
                postData('/v1/inquire/getInquireTemplateList', d).then((res) => {
                    if(res.res_code == 1) {
                        ...
                        this.hint = `<span style="letter-spacing: 0.93px;white-space: nowrap">共<span style="color: #4098ff">${this.total}</span>/${res.data.list.length}条</span>`
                    }
                })
            },
            getClassify() {
            ...
            getCategory() {
                return  postData('/v1/common/getInquireCategory').then(res => {
                    if(res.res_code == 1) {
            ...
            modifyInput(val, data, el) {
                if(el) el.path[0].blur()
                    ...
                    postData('/v1/inquire/addInquireCategory', {title: val}).then(res => {
                        if(res.res_code == 1) this.getCategory()
                    })
                }else if(val && !data.is_create) {
                    postData('/v1/inquire/modifyInquireCategory', {id: data.id, title: val}).then(res => {
            ...
            remove (root, node, data) {
                this.$Modal.confirm({
                    ...
                    content: '删除后不会对已发布的病例造成影响',
                    onOk: () => {
                        postData('/v1/inquire/removeInquireCategory', {id: data.id}).then(res => {
                            if(res.res_code == 1) this.getClassify()
                        })
