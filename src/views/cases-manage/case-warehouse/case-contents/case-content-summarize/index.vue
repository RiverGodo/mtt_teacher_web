<template>
    <div class="case-content-summarize" id="content">
        <Form ref="formValidate" class="case-form" :model="formInline" :rules="ruleValidate">
            <div v-if="isCheck" style="position: absolute;left: 0;right: 0;top: 0;bottom: 38px;z-index: 1" class="aaa"></div>
            <FormItem>
            ...
            <FormItem label="民族" style="display: inline-block; width: 49%;" prop="patient_nation">
                <Select style="width: 262px;" v-model="formInline.patient_nation" placeholder="患者民族">
                    <Option v-for="(item, index) in nationList" :value="item.id" :key="index">{{ item.nation }}</Option>
                </Select>
            </FormItem>
            ...
            <FormItem v-if="formInline.patient_gender == 'Female'" label="生育情况" style="display: inline-block; width: 49%;" prop="is_fertility">
             ...
             <FormItem class="case-postpartum" v-if="formInline.is_fertility == 1 || formInline.is_fertility == 2" style="display: inline-block; width: 49%">
                <template slot="label"><span style="color: #ff3300;margin-right: 4px;line-height: 1">*</span>{{formInline.is_fertility == 1 ? '怀孕' : '产后'}}时间</template>
        ...
        data() {
            return {
                ...
                ruleValidate: {
                    ...
                    cardinal_symptom: [{ required: true, message: '请输入患者主要病症', trigger: 'change'}],
                    chief_complaint: [{ required: true, message: '请输入患者对病情描述', trigger: 'change'}],
                },
                symptom_list: [],
                patient_list: [],
...
content.scrollIntoView()
            ...
            addAndEdit(d, save){
                PostData('/v1/case/setPatientInfo', d).then((r) => {
                    if(r&&r.res_code == 1){
            ...
            handleNext(save) {
                ...
                console.log(this.formInline,"wsq");
                ...
                this.$refs.formValidate.validate((valid) => {
                    if(valid&&this.isPostpartum()) {
                        ...
                        da1.patient_gender = da1.patient_gender == "Female" ? 2 : 1
                        da1.patient_age = data.patient_age
                        da1.patient_age_type = parseInt(data.patient_time_type)
                        da1.patient_nation_id = da1.patient_nation
                        da1.patient_marital_status = parseInt(da1.patient_marital_status)
                        da1.medical_history_representor = parseInt(da1.medical_history_representor)
                        da1.is_fertility = da1.is_fertility ? da1.is_fertility : 0
                        // da1.patient_marital_status = da1.patient_marital_status == "未婚" ? 2 : da1.patient_marital_status == "已婚" ? 1 : 3
                        if (da1.postpartumWeeks) {
                            da1.fertility_time_week = da1.postpartumWeeks
                        }else{
                            da1.fertility_time_week = 0
                        }
                        if (da1.postpartumDay) {
                            da1.fertility_time_day = da1.postpartumDay
                        } else {
                            da1.fertility_time_day = 0
                        }
...
content.scrollIntoView()
            ...
            getSel(){
                PostData('/v1/common/getNationList').then((r) => {
        ...
        mounted() {
            ...
            if(sessionStorage.getItem('CaseContentSum')){
                ...
                this.formInline = d
                this.formInline.patient_time_type = d.patient_time_type
                this.formInline.patient_age = d.patient_age
            ...
            if (this.$route.query.id) {
                postData('/v1/case/getPatientInfo',{case_id:parseInt(this.$route.query.id)}).then(res => {
                    let d = res.data
                    this.formInline = d
                    this.formInline.patient_time_type = String(d.patient_age_type)
                    this.formInline.patient_age = d.patient_age
                    this.formInline.patient_gender = d.patient_gender == 1 ? "Male" : "Female"
                    this.formInline.patient_nation = d.patient_nation_id
                    this.formInline.patient_marital_status = String(d.patient_marital_status)
                    this.formInline.medical_history_representor = String(d.medical_history_representor)
                    this.formInline.postpartumWeeks = d.fertility_time_week
                    this.formInline.postpartumDay = d.fertility_time_day
                })
            }
