  ...
  <div style="height: 100%;">
    <div class="header">
      ...
      <div v-if="!isCheck" class="header-right">
        ...
        <div class="header-right-box" @click="handleSumit" v-if="progress == 6">
          <img class="header-right-img" :src="content_submit" alt />
          <div class="header-right-font">提交</div>
        </div>
      </div>
      <div v-else class="header-right">
        <!-- <div class="header-right-box" @click="copyCase">
          <Button style="height: 50px;border-radius: 0;" @click="copyCase" type="primary">复制病例</Button>
        </div> -->
      </div>
    </div>
    <div class="content">
      ...
      <!-- 病例信息 -->
      <content-message ref="content_message" v-if="progress == 1" @handleNavigate="handleNavigate" />
      <!-- 患者概述 -->
      <content-summarize
        ref="content_summarize"
        v-if="progress == 2"
        @handleNavigate="handleNavigate"
      />
      <!-- 创建诊断 -->
      <content-diagnose
        ref="content_diagnose"
        v-if="progress == 3"
        @handleNavigate="handleNavigate"
      />
      <!-- <arrangement style="width: 100%;" v-if="progress == 4" @handleNavigate="handleNavigate" /> -->
      <!-- 诊疗详情 -->
      <DetailsDiagnosis v-if="progress == 4" @handleNavigate="handleNavigate" />
      <!-- 提纲查看 -->
      <OutlineSee style="width: 100%;" v-if="progress == 5" @handleNavigate="handleNavigate" />
      <!-- 评分设置 -->
      <ScoreSet v-if="progress == 6" @handleNavigate="handleNavigate" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      isCheck: (state) => state.product.isCheck,
    }),
  },
  methods: {
    copyCase() {
    ...
    opinionNavigate(index) {
      this.maxProgress = +sessionStorage.getItem("CaseActiveMaxStep");
      if (index == 0)
    ...
    handleNavigate(index) {
      if (parseInt(sessionStorage.getItem("CASEID") ? sessionStorage.getItem("CASEID") : this.$route.query.id)) {
        PostData("/v1/case/getStep", {
          id: parseInt(sessionStorage.getItem("CASEID") ? sessionStorage.getItem("CASEID") : this.$route.query.id),
        }).then((res) => {
          if (index > res.data.step || !res.data.step){
            PostData("/v1/case/setStep", {
              id: parseInt(sessionStorage.getItem("CASEID") ? sessionStorage.getItem("CASEID") : this.$route.query.id),
              step: index,
            }).then((res) => {
              if (res.res_code == 1) {
    ...
    handleSave() {
      (async () => {
        ...
        let CaseContentSum = JSON.parse(
          sessionStorage.getItem("CaseContentSum")
        );

        let state = undefined;

        if (CaseContentSum) {
          state = +CaseContentSum.state;
        }
        console.log(CaseContentSum);
        if (!state || state == 1) {
          let d = sessionStorage.getItem("CaseActiveStep") || 1;
          await $CasePreservation.$emit("CasePreservation", d);
          // await PostData('case/modifyCase',{id: +sessionStorage.getItem('CASEID'), state: 0}).then((r) => {
      ...
      if (this.maxProgress == 6) {
        ...
        if (state === 3) this.$Message.info("该病例已通过审核");
          ...
          PostData("/v1/case/modifyCaseInfo", {
            ...
            } else $CaseSummarys.$emit("CaseSummarys");
          });
        }
  ...
  mounted() {
    if (sessionStorage.getItem("CaseActiveStep"))
      this.progress = +sessionStorage.getItem("CaseActiveStep");
    if (sessionStorage.getItem("CaseActiveMaxStep"))
      this.maxProgress = +sessionStorage.getItem("CaseActiveMaxStep");
    this.$store.commit(
      "setCheck",
      this.$route.query ? this.$route.query.isCheck : false
    );
  },
};
</script>
