<template>
    <div class="scene-des">
        ...
        <Input :disabled="isCheck" class="inputs" v-model="scene" type="textarea" :rows="5" placeholder="请生动形象描述当前场景" />
        <div>
            <div>
                <span>操作限时</span>
            ...
            <div style="margin-top: 10px;text-align: left;">
                <InputNumber :disabled="isCheck" class="times" :min="0" v-model="times"></InputNumber>
                <span>分钟</span>
            </div>
        </div>
        <Button :disabled="isCheck" class="scene-btns" @click="Preservation">保存</Button>
    ...
    export default {
        ...
        methods: {
            ...
            Preservation(){
                ...
                if(this.scene){
                    if(this.times) {
                        PostData('/v1/case/setScenceDesc',d).then((r) => {
                            if(r.res_code == 1){
            ...
            getDateils(){
                PostData('/v1/case/getScenceDesc', {case_examine_frequency_id: this.diagnosisId}).then((r) => {
                    if(r.res_code == 1) {
