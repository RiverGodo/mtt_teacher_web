<template>
  <div class="operation-right" id="content"  ref='content'>
    <SeeCase
      v-if="type == 3"
      :showModal="showModal"
      :type="filesType "
    ...
    <div class="main-right" :class="setClass">
      ...
      <div v-if="isAdd || isEdit">
        <div :class="allAdd ? 'main-types' : 'main-type1'" ref="MainView" style="height:400px;overflow: auto;" class="main-view">
          <div v-if="type == 1">
            <div class="item-form">
            ...
            <div v-show="changetypes == 1" style="height: 80px;display: flex;align-items: flex-start;" class="item-form">
              <span class="item-type">解析</span>
              <Input type="textarea" :rows="3" :disabled="isCheck" v-model="parsing" placeholder="请填写解析" class="case-type" />
              <Poptip word-wrap width="200" trigger="click" placement="left">
                <img style="position: absolute;cursor: pointer;margin-left: 10px;top: -10px;" :src="infot" alt="">
                <div class="imgsloat" slot="content">
                  <div class="ddes">请解释，该问诊为【得分项】的理由。</div>
                </div>
              </Poptip>
            </div>
          </div>
          <div class="phy" v-if="type == 2&&isEdit || type == 3&&isEdit">
            ...
            <div v-if="type == 2&&isEdit" class="right-content-btn">
              <div style="width: 300px;">
                <div
                  v-show="item.result_media_url" 
                  v-for="(item, index) in upload_file"
                  @click="seeMediaFile(item, fileType)"
                  :key="index"
                  class="file_item"
                >
                <img style="width: 100%" v-if="item.result_media_type == 'Picture'" :src="mediaUrl + item.result_media_url"/>
                <video controls width="100%" v-if="item.result_media_type == 'Video'" :src="mediaUrl + item.result_media_url"/>
                <audio controls style="width: 100%" v-if="item.result_media_type == 'Audio'" :src="mediaUrl + item.result_media_url"/>
            ...
            <div v-if="type != 3&&changetypes == 1">
              <div style="display: flex;align-items: center;">
                <span style="float: left;margin-top: 20px;margin-bottom: 10px;">解析</span>
                <Poptip word-wrap width="200" trigger="click" placement="right">
                  <img style="cursor: pointer;margin-left: 10px;margin-top: 15px;" :src="infot" alt="">
                  <div class="imgsloat" style="top: -20px;left: 170px;" slot="content">
                    <div class="ddes">请解释，该查体为【得分项】的理由。</div>
                  </div>
                </Poptip>
              </div>
              <Input type="textarea" :rows="2" :disabled="isCheck" v-model="parsing" placeholder="请填写解析" class="case-type" />
            </div>
          </div>
          <div v-if="type == 3&&isEdit">
            ...
            >
              <div
                :class="media ? 'show-media' : 'hide-media'"
                style="width: 300px;"
              >
                <div class="file_item" v-if="media.url != mediaUrl+'null'" @click="seeFile(media)">
                  <!-- {{media}} -->
                  <!-- <img class="media-img" :src="wordImg" />
                  <div class="upload-name">{{media&&media.media_name}}</div> -->
                <img style="width: 100%" v-if="media.media_type == 'Picture'" :src="media.url"/>
                <video controls width="100%" v-if="media.media_type == 'Video'" :src="media.url"/>
                <audio controls style="width: 100%" v-if="media.media_type == 'Audio'" :src="media.url"/>
                  <Icon
                    color="#4098FF"
                    class="upload-delete"
          ...
          <div style="margin:0 20px;" v-if="type == 3&&changetypes == 1&&isEdit">
            <div style="display: flex;align-items: center;">
                <span style="float: left;margin-top: 20px;margin-bottom: 10px;">解析</span>
                <Poptip word-wrap width="200" trigger="click" placement="right">
                  <img style="cursor: pointer;margin-left: 10px;margin-top: 15px;" :src="infot" alt="">
                  <div class="imgsloat" style="top: -20px;left: 170px;" slot="content">
                    <div class="ddes">请解释，该辅检为【得分项】的理由。</div>
                  </div>
                </Poptip>
            </div>
            <Input type="textarea" :rows="2" :disabled="isCheck" v-model="parsing" placeholder="请填写解析" class="case-type" />
          ...
          <div class="term" style="margin-left: 40px;margin-bottom: 15px;display: flex;align-items: flex-start;" v-if="setItemType && changetypes == 2 && item_type != 3">
            <span class="term-title grey-regular4">解析</span>
            <div style="width: 360px;position: relative;display: flex;align-items:flex-start;">
              <Input type="textarea" :rows="3" :disabled="isCheck" v-model="parsing" placeholder="请填写解析" class="case-type" />
            </div>
            <Poptip word-wrap width="200" trigger="click" placement="left">
                <img style="cursor: pointer;margin-left: 10px;" :src="infot" alt="">
                <div class="imgsloat" style="top: -20px;left: 170px;" slot="content">
                  <div class="ddes">请解释，该<span v-if="type == 1">问诊</span><span v-else-if="type == 2">查体</span><span v-else-if="type == 3">辅检</span><span v-else>治疗</span>为【干扰项】的理由。</div>
                </div>
            </Poptip>
          </div>
          <!-- {{item_type}} -->
...
export default {
  ...
  data() {
    return {
      ...
      diagnosisFm: { reason: "", weight: 0, operation: "", other_name: "",parsing:"" },
      ...
      fileText: [
        ...
        "* 支持jpg/png图片、mp3音频、mp4/mov/avi视频，只能上传一个文件",
      ],
      fileDir:
  ...
  watch:{
    item_type(item){
      console.log(this.item_type,"wsq");
      if (!this.item_type) {
        // this.item_type = 1
      }
    }
  ...
  methods: {
    ...
    cascaderType(v, selectedData) {
      ...
      if (this.isAddForm()) this.getPhyListZlxq();
    ...
    getPhyListZlxq(){
      ...
      let d = {
        ...
        keyword: this.serach,
        frequency_id: this.diagnosisId,
        parent_id:da.first_id,
        category_id:this.modelType[0]
      };
      let url = ["/v1/case/getPhysicalTemplateList_zlxq", "/v1/case/getAssistantTemplateList_zlxq"];
      PostData(url[this.type - 2], d).then((r) => {
        if (r && r.data) {
          this.phyList = r.data.list;
          this.actPhy = "";
        }
      });
    },
      ...
      if (id1 == 2) {
        ...
        if (this.modelType.length == 2) {
          let d = {
            ...
            type: this.modelType[1] == 9 ? 1 : 2,
            keyword: this.serach,
            frequency_id: this.diagnosisId,
            ...this.case_id,
            case_treatment_id: +sessionStorage.getItem("CaseTreatment"),
          };
          PostData("/v1/treatment/getMedicineList", d).then((r) => {
            if (r && r.data) {
              this.phyList = r.data.list;
              if (r.data.list.length == 0) this.$Message.info("暂无数据！");
              this.actPhy = "";
            }
          });
        }
    ...
    setUploadImg(val) {
      ...
      this.upload_file.push({
        ...
        result_media_type:this.fileType
      });
    },
    uploadImg(val) {
      let imgs = ["jpeg", "jpg", "png"];
    ...
    selPhyItem(t, i) {
      console.log(t,i);
      // console.log(t.is_check == 0 ? t.is_check == 0 : t.state == 0);
      if (t.is_check == 0 ? t.is_check == 0 : t.state == 0) {
        this.actPhy = i;
        this.actItem = t;
    ...
    getPhyData() {
      ...
      let url = ["v1/common/getPhysicalCategory", "/v1/common/getAssistantCategory"];
      PostData(url[this.type - 2], d).then((r) => {
        if (r && r.data) {
          // this.phyList = r.data;
          this.actPhy = "";
    ...
    getSel() {
      PostData("/v1/common/getInquireCategory", {
        ...this.case_id,
        frequency_id: this.diagnosisId,
      }).then((r) => {
    ...
    addInterrogation(d) {
      let params = {
        ...d,
        frequency_id: this.diagnosisId,
        category_id:d.talk_group,
        supporting_basis:[],
        non_supporting_basis:[]
      };
      params.case_diagnoses.forEach(element => {
        if (element.basis_type == "SupportingBasis") {
          params.supporting_basis.push(element)
        }else{
          params.non_supporting_basis.push(element) 
        }
      });
      let saveadd = params.talk_id
      let questionsids = JSON.parse(sessionStorage.getItem('Question')) ? JSON.parse(sessionStorage.getItem('Question')) : []
      // console.log(questionsids,"wss");
      if (this.isAdd && params.talk_id) {
        delete params.talk_id;
      }
      // console.log(this.type);
      if (this.title == "编辑") {
        PostData("/v1/inquire/modifyInquire", {...params,id:params.talk_id}).then((r) => {
          if(r.res_code == 1)this.addSuccess(r),questionsids.push(saveadd),sessionStorage.setItem("Question",JSON.stringify(questionsids));
        });
      }else{
        PostData("/v1/inquire/addInquire", params).then((r) => {
          if(r.res_code == 1)this.addSuccess(r),questionsids.push(saveadd),sessionStorage.setItem("Question",JSON.stringify(questionsids));
        });
      }
    },
    ...
    verInterrogationChild(name, text){
    ...
    sendInterrogation(d) {
      if (
        ...
        this.item_type != 3 ? this.verInterrogation("parsing", "请输入解析！") : true &&
    ...
    physicalExam() {
      ...
      if (this.actItem && this.isEdit) {
        ...
        d.item_type = parseInt(this.item_type || this.tabindex);
        ...
        if (this.isEdit) {
          ...
          // console.log(this.upload_file,this.upload_file.length,this.fileType);
          if (this.upload_file && this.upload_file.length > 0) {
            d.result_media_type = 1;
          } else {
            d.result_media_type = 0;
          }
          d.deduct_reason = this.deduct_reason;
          d.deduct_weight = String(this.deduct_weight);
        ...
        let params = {
          ...
          explain:this.parsing,
          ...this.case_id,
          case_assistant_method_id:d.case_physical_method_id,
          supporting_basis:[],
          non_supporting_basis:[]
        };
        ...
        params.case_diagnoses.forEach(element => {
          if (element.basis_type == "SupportingBasis") {
            params.supporting_basis.push({
              case_diagnose_id:element.case_diagnose_id,
              weight:element.weight
            })
          } else {
            params.non_supporting_basis.push({
              case_diagnose_id:element.case_diagnose_id,
              weight:element.weight
            })
          }
        ...
        params.type = params.datas[0].result_media_type == "Picture" ? 1 : params.datas[0].result_media_type == "Audio" ? 2 : 3
        // console.log(this.tabindex);
        if (sessionStorage.getItem('ADDOREDIT') == "add") {
          PostData("/v1/physical/addPhysical", params).then((r) => {
            if (r && r.res_code == 1) {
          ...
          params.url = datas[0].result_media_url
          params.basis = []
          PostData("/v1/physical/modifyPhysical", params).then((r) => {
            if (r && r.res_code == 1) {
              if (this.isAdd) this.actItem = null;
              this.addSuccess(r);
            }
    ...
    supplementaryExam() {
      // console.log(this.item_type);
      if (this.item_type != 3 ? this.verInterrogation("parsing", "请输入解析！") : true ) {
          let d = {};
          if (this.actItem && this.actItem.id) {
            ...
            console.log(params);
            params.supporting_basis = []
            params.non_supporting_basis = []
            params.case_diagnoses.forEach(element => {
              if (element.basis_type == "SupportingBasis") {
                params.supporting_basis.push(element)
              } else {
            ...
            if (params.media && params.media.length != 0) {
              if (params.media.url) {
                let urls = params.media.url.split("/")
                params.url = urls[urls.length-2] + "/" + urls[urls.length-1]
                params.file_name = params.media.media_name
                params.result_attachment_type = 1
              }else if(params.media[0].abbreviation){
                params.result_attachment_type = 2
                params.table_content = JSON.stringify(params.media)
              }
            }else{
               params.result_attachment_type = 0
            }
            if (sessionStorage.getItem('ADDOREDIT') == "add") {
              PostData("/v1/assistant/addAssistant", params).then((r) => {
                if (r.res_code == 1) this.addSuccess(r);
              });
            } else {
              params.basis = []
              PostData("/v1/assistant/modifyAssistant", params).then((r) => {
                if (r.res_code == 1) this.addSuccess(r);
              });
            }
          } else this.$Message.info("请选择类别！"); 
    ...
    pleaseChoose(fn) {
      ...
      if (this.isEdit) {
        if (
          this.verification(2) &&
          (this.item_type === 1 ? this.verDiagnosisFm("reason", "请输入应用理由！") : true) &&
          (this.item_type === 1 ?  this.verInterrogationChild("parsing", "请输入解析！") : (this.item_type === 2  ? this.verInterrogation("parsing", "请输入解析！") : true)) &&
          this.isNursingBasis()
        )
          fn();
      }
    ...
    pleaseWeight(name, text) {
      console.log(this.diagnosisFm,name,text);
      let bool = false;
      text = text || "请选择得分权重！";
      name = name || "weight";
      if (
    ...
    //增加其他
    addOther() {
      console.log(this.diagnosisFm);
      if (
        (this.pleaseWeight("name", "请输入治疗名称!")) &&
        (this.item_type === 1 ? this.verDiagnosisFm("reason", "请输入应用理由！") : true) &&
        (this.item_type === 1 ?  this.verInterrogationChild("parsing", "请输入解析！") : (this.item_type === 2  ? this.verInterrogation("parsing", "请输入解析！") : true)) &&
        this.isNursingBasis()
      ) {
        ...
        let explainlist = "";
        if (this.tabindex == 1) {
          explainlist = this.diagnosisFm.parsing ? this.diagnosisFm.parsing : this.parsing
        } else {
          explainlist = this.parsing ? this.parsing : this.diagnosisFm.parsing
        }
        let d = {
          ...this.case_id,
          id: +sessionStorage.getItem("CaseTreatment"),
          name: this.diagnosisFm.name,
          element_id: this.element_id,
          reason_for_using: this.diagnosisFm.reason,
          weight: this.item_type == 2 ? 0 : this.diagnosisFm.weight,
          item_type: this.item_type,
          deduct_reason: this.deduct_reason,
          deduct_weight: this.deduct_weight,
          explain:explainlist,
        ...
        let url = this.isAdd ? "/v1/treatment/addOther" : "/v1/treatment/modifyOther";

        PostData(url, { ...d, frequency_id: this.diagnosisId }).then(
          (r) => {
            if (r.res_code == 1) {
              if (this.isAdd) this.actItem = null;
              this.addSuccess(r);
    ...
    addOperation() {
      ...
      if (
        ...
        if (this.isEdit) d.id = t.id;
        let url = this.isAdd ? "/v1/treatment/addOperation" : "/v1/treatment/modifyOperation";
        PostData(url, { ...d, frequency_id: this.diagnosisId }).then(
          (r) => {
            if (r.res_code == 1) {
              if (this.isAdd) this.actItem = null;
    ...
    addNursing() {
      if (
        ...
        let da = {
          ...
          id: +sessionStorage.getItem("CaseTreatment"),
          reason_for_using: this.diagnosisFm.reason,
          diet_ids: this.setFiled(2, "diet_id", "diet"),
          manipulations: this.setFiled(5, "id", "content"), //{manipulation, manipulation_id},//操作 case_treatment_manipulation(多选)
          item_ids: this.setFiled(4, "nursing_id", "nursing"), //{nursing, nursing_id},//护理 case_treatment_nursing(多选)
          nursing_level: (d[0] && d[0].label) || null,
          nursing_level_id: this.drugContents[0], //护理等级case_treatment_nursing_level
          routine: this.drugContents[1], //护理常规 case_treatment_nursing_routine
        ...
        if (this.isEdit){
          da.id = this.actItem.id;
          PostData("/v1/treatment/modifyNursing", da).then((r) => {
            if (r.res_code == 1) this.addSuccess(r);
          });
        }else{
          PostData("/v1/treatment/addNursing", da).then((r) => {
            if (r.res_code == 1) this.addSuccess(r);
          });
        } 
      }
    ...
    //增加药品
    addDrugs() {
          // console.log(this.diagnosisFm.parsing,"111111");
          // console.log(this.parsing,"222222");
          let t = this.actItem;
          ...
          let da = {
            ...
            reason_for_using: this.diagnosisFm.reason,
            weight: this.diagnosisFm.weight || 0,
            explain:explainlist,
            case_treatment_id:+sessionStorage.getItem("CaseTreatment")
          };
          let d = {};
          ...
          // console.log(d,t);
          let url = this.firstAddDrugs ? "/v1/treatment/addMedicine" : "/v1/treatment/modifyMedicine";
          PostData(url, { ...d,medicine_frequency_id:this.drugContents[2] }).then((r) => {
            if (r.res_code == 1) {
              this.firstAddDrugs = false;
    ...
    determine() {
      ...
      //获取最外层content
      let backTopNode = this.$refs.content;
      backTopNode.scrollIntoView()
    },
    editInitDiagnose(t, type1, type2, name) {
      // console.log(t, type1, type2, name);
    ...
    //编辑诊断
    getTalkDiagnose(item,t) {
      this.mainData.check = false
      if (t.basis) {
          t.basis.forEach(element => {
            element.basis_type = element.type == 1 ? 'SupportingBasis' : 'NonSupportingBasis'
            if (element.case_diagnose_type == 1) {
              element.types = 'PrincipalDiagnosis'
            }else if(element.case_diagnose_type == 2){
              element.types = 'SecondaryDiagnosis'
            }else if(element.case_diagnose_type == 3){
              element.types = 'DifferentialDiagnosis'
            }
          })
          // console.log(t.basis);
          // t.basis.forEach(element => {
          ...
          //   }
          // });
          // console.log(t.basis.length);
          for (let i = 0; i < t.basis.length; i++) {
            if (
              t.basis[i].types == "PrincipalDiagnosis" &&
              t.basis[i].basis_type == "SupportingBasis"
            ) {
              this.mainData.check = true;
              this.mainData.weight = t.basis[i].weight || 0;
            }
            if (t.basis[i].basis_type == "SupportingBasis") {
            ...
            if (t.basis[i].basis_type == "NonSupportingBasis") {
              this.editInitDiagnose(
                t.basis[i],
                "SecondaryDiagnosis",
                "NonSupportingBasis",
      ...
      // let url = ["getInquireElementDetail", "getPhysicalDiagnose", "getAssistDiagnose"][
      //   this.type - 1
      // ];
    ...
    getDetail() {
      ...
      PostData("/v1/case/getDiagnoseList", {
        ...this.case_id,
        frequency_id: this.diagnosisId,
      }).then((r) => {
        if (r && r.data) {
    ...
    async editType2(item, t, get_result_type) {
      ...
      // let url = type ? "/v1/case/getPhysicalElementDetail" : "case/getAssistResultById";
      let params = { element_id: t.id, case_examine_frequency_id: this.diagnosisId };
      // console.log(params);
      // if (!params.id) {
      //   return;
      // }
      ...
      //     this.parsing = r.data.explain;
      //     if (get_result_type == "basis") {
      //       // 拼接 phyItem(编辑时)
      //     this.phyItem = this.actItem.category_name + ">" + this.actItem.name
      ...
      //         this.media = d;
      //         this.media.media_type = r.data.media.type
      //         this.media.url = this.mediaUrl + r.data.media.report
      ...
      //   }
      // });
      // console.log(t.item_type);
      let filetype = ['','Picture','Audio','Video']  
      this.actItem = t
      this.thirdName = t.name;
      this.parsing = t.explain;
      this.phyItem = t.name;
      this.upload_file = [
        {
          result_media_type:filetype[t.type],
          result_media_url:t.url
        }
      ]
      this.deduct_weight = t.deduct_weight
      this.deduct_reason = t.deduct_reason
      this.media = {
        media_type:filetype[t.type],
        url:this.mediaUrl + t.url
      }
      this.assisTable = t.content ? JSON.parse(t.content) : []
      // this.item_type = t.item_type
      if (t.result_attachment_type == 1) {
        this.data_type = "ImageData"
      } else if(t.result_attachment_type == 2) {
        this.data_type = "ExaminationReport"
      }else{
        this.data_type = "None"
      }
      
      this.getTalkDiagnose(item,t);
      this.findIdentifyElementCodeShow(item, t);
    },
    editType1(item, t) {
      this.findIdentifyElementCodeShow(item, t);
      this.element_id = item.id;
      this.question_index = t.basis;
      this.$nextTick(() => {
        // console.log(t);
        this.caseType = this.isTerm() ? t.category_id : t.category_id;
        let current_select = this.selectList.filter((item) => 
            item.talk_group == this.caseType
        );
        if (current_select.length > 0) {
          this.category_id = current_select[0].id;
        }
      ...
      this.parsing = t.explain;
      this.anyQuestions = t.question;
      if (t.basis) this.getTalkDiagnose(item,t);
      if (t.item_type == 3) {
        this.deduct_reason = t.deduct_reason;
        this.deduct_weight = t.deduct_weight;
      }
    ...
    findIdentifyElementCodeShow(item, t) {
      // console.log(t,this.IdentifyList);
      this.identify_element_code_sel = []
    ...
    editType4(item, t) {
      ...
      if (t.type == "treat") {
        ...
        this.drugContents = [
          t.nursing_level_id,
          t.routine,
          this.getIds(t.diet_ids),
          t.position_id,
          this.getIds(t.item_ids),
          this.getIds(t.manipulations, "id"),
        ];
      ...
      if (t.type == "medicine") {
        ...
        this.diagnosisTypes = 2;
        this.medicineNum =
          t.medicine_type == "西药" ? 6 : t.medicine_type == "中药" ? 9 : 0;
        this.drugContents = [t.usage_id, t.dosage, t.medicine_frequency_id];
        this.diagnosisTitle = t.name;
      }
      this.actItem = t;
      this.diagnosisFm.reason = t.reason_for_using;
      this.diagnosisFm.weight = t.weight;
      this.diagnosisFm.parsing = t.explain;
      this.diagnosisFm.name = t.name || t.operation
      this.parsing = t.explain;
      if (t.item_type == 3) {
        this.deduct_reason = t.deduct_reason;
        this.deduct_weight = t.deduct_weight;
    ...
    setEdit(item, t, typePane, get_result_type) {
      // this.clear();
      this.changetypes = this.tabindex
      // console.log(item, t, typePane, get_result_type,"wsq");
      this.clearData("secondData");
      this.clearData("secondData2");
      this.clearData("threeData");
      this.clearData("threeData2");
    ...
    getPhyTypes(url) {
      PostData(url, { ...this.case_id, frequency_id: this.diagnosisId, parent_id:0 }).then(
        (r) => {
          if (r && r.data)
  ...
  mounted() {
    ...
    if (this.type == 2) this.getPhyTypes("/v1/common/getPhysicalCategory");
    if (this.type == 3) this.getPhyTypes("/v1/common/getAssistantCategory");
