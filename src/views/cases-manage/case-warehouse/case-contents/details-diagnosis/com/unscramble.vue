<template>
    <div class="analysis">
        <div class="analysis-content">
            <Input :disabled="isCheck" v-model="treatment_principle" type="textarea" :rows="15" placeholder="请输入解读分析内容" />
        ...
        methods: {
             add(){
                 ...
                 if(this.treatment_principle){
                    PostData('/v1/case/setUnscramble', d).then((r) => {
                        if(r&&r.res_code == 1) this.$Message.info("保存成功");
                    })
                 }
             },
             getData(){
                PostData('/v1/case/getUnscramble',{case_examine_frequency_id: this.diagnosisId, case_id: this.case_id, type: parseInt(this.type)}).then((r) => {
                    if(r&&r.res_code == 1) {
