<template>
    <div class="main">
        <div class="tabs-view tabs-content">
            <Tabs @on-click="tabs" v-model="casePane" type="line" size="small" class="tabs">
                <TabPane name="1" :label="setLabel">
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 1" :tabindex="casePane" :type="page" pane="essential1" />
                </TabPane>
                <TabPane name="2" label="干扰项/扣分项">
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 2" :tabindex="casePane" :typePane="typePane" :type="page" pane="details1" />
                </TabPane>
        ...
        <div class="content let-content">
            <OperationRight :diagnosisId="diagnosisId" :tabindex="casePane" :type="page" />
        </div>
        <Modal v-model="outlineModal" :footer-hide="true" title="查看提纲" width="1100" @on-cancel="modalCancel">
            <outlines ref="InterrogationOutline" :dragcanvas="true" v-if="outlineModal" style="height: 700px;"/>
        </Modal>
    </div>
</template>
        ...
        methods: {
            setOutlinesData(){
                PostData('/v1/outline/getoutlineDetail', {id: parseInt(JSON.parse(sessionStorage.getItem('ACTIVEOUTLINE')).id)}).then(res => {
                    if(res.res_code == 1) {
                        console.log(JSON.parse(res.data.outline).nodes);
                        this.$refs.InterrogationOutline.data = JSON.parse(res.data.outline)
                        this.$refs.InterrogationOutline.graph.read(JSON.parse(res.data.outline))
                        this.$refs.InterrogationOutline.updateCanvasSize()
                    }
                })
            },
            tabs(name){
