<template>
    <div class="treatment-right">
        ...
        <div class="treatment-nursing">
            <nursing-drugs :weightTit="titles" :itemType="1" @nursing-drugs-data="nursingDrugsData" :diagnosisTitle="diagnosisTitle" :medicineNum="medicineNum" :diagnosisData="diagnosisData" :isEdit="isEdit" :diagnosisFm="diagnosisFm" :inputContents="inputContents" :drugContents="drugContents" :type="isNursing" />
        </div>
        <div v-if="isEdit" class="case-btn-box">
            <Button v-if="show" @click="backAdd" class="back-btn">返回添加</Button>
    ...
    export default {
        ...
        methods: {
            ...
            setWeights(d){
                ...
                // let name = this.type == 5 ? 'deduct_weight' : 'weight'
                // d[name] = this.diagnosisFm.weight
                // 此处修改所有类型均添加一下两字段
                d.deduct_weight = this.diagnosisFm.weight
                d.weight = this.diagnosisFm.weight
            },
            addOperation(){
            ...
            addOther(){
                if(this.pleaseWeight('other_name','请输入治疗名称!')&&this.verDiagnosisFm('reason', '请输入应用理由！')&&this.pleaseWeight()){
                    let t = this.actItem
                    let d = {
                        ...this.case_id,
                        ...
                        reason_for_using: this.diagnosisFm.reason,
                        explain:this.diagnosisFm.parsing,
                        ...this.setItemType()
                    }
                    this.setWeights(d)
                    if(this.isEdit) d.id = t.id
            ...
            setEdit(item, t){
                ...
                this.diagnosisFm.weight = this.type == 5 ? t.deduct_weight : t.weight
            }
        },
        setWeight(val){
            this.diagnosisFm.weight = val
        },
        beforeDestroy () {
            $CaseEdit.$off('CaseEdit', this.setEdit)
            $CaseScoreWeightDeduction.$emit('CaseScoreWeightDeduction', this.setWeight)
        },
        mounted() {
            $CaseScoreWeightDeduction.$emit('CaseScoreWeightDeduction', this.setWeight)
