<template>
    <div class="score-detail" >
       <div class="score-titles">{{type == 5 ? '添加扣分项' : '添加依据'}}</div>
           ...
           <div class="score-content">
               <div v-if="list&&list.length>0" class="collapse">
                    <div v-for="(t,i) in list" :key="i" class="pane" :style="{'margin-bottom': t.show ? 0 : '12px'}">
                        <div v-show="t.chilrdren && t.chilrdren.length" class="item" @click="togg(t, i)">
                            <div class="item-title">{{t.talk_group || t.first_name || t.name}}</div>
                            <div class="right">
                                <img class="sel" :src="t.show ? selClose : selOpen" />
                            </div>
                        </div>
                        <div class="details" v-show="t.show">
                            <div class="content"  v-for="(it, l) in t.chilrdren" :key="l" :disabled="it.is_check" :class="setClass(it)">
       ...
       <div class="case-btn-box" v-if="type == 2&&isAdd&&!isCheck || type == 3&&isAdd&&!isCheck">
            ...
            <Button v-if="type == 3" :disabled="isDisable" class="case-btn" @click="addExclude" type="primary">排除依据</Button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            item1(){
                return this.tabs == 4 ? 'content' : this.tabs == 1 ? 'question' : (this.tabs == 2 || this.tabs == 3) ? 'third_name'  : ''
            },
            isDisable(){
        ...
        methods: {
            ...
            setClass(it){
                if(it.is_check || this.isCheck) return 'disabled-content-detail'
                else return (it.id === this.activeId || it.first_id === this.activeId)  ? 'act-content' : 'de-content'
            },
            getListUrl(){
                return ['/v1/case/getCaseInquireCategoryList', '/v1/case/getCasePhysicalCategoryList', '/v1/case/getCaseAssistantCategoryList', 'case/getFirstListByTab'][this.tabs - 1]
            },
            getTalkUrl(){
                return ['/v1/case/getCaseInquireCategoryList', 'case/getPhysicalResultListForScore', 'case/getAssistResultListForScore', 'case/getListByType'][this.tabs - 1]
            },
            ...
            addDeduction(t, it, i, l, r, bool){
                let urls = {"operation": 'modifyOperation', "medicine": 'modifyMedicine', "treat": 'operateNursing'}
                it.item_type = 3
                it.case_id = +sessionStorage.getItem('CASEID')
                it.case_treatment_id = +sessionStorage.getItem('CaseTreatment')
                it.element_id = it.id
                PostData('case/' + urls[it.type], it).then((r) => {
            ...
            allAdd(t, it, i, l, bool){
                if(t&&it){
                    ...
                    if(this.tabs == 4) this.addDeduction(t, it, i, l, bool)
                }else this.$Message.info('请选择依据！');
            },
            add(t, it, i, l){
                // 检测(除了扣分项外)
                // “干扰项”不可以直接在 评分设置中 添加依据，点击“干扰项”时，提示：请在【诊疗详情】中进行设置
                if(this.type!=5 && it.item_type == 2){
                    this.$Message.error("请在【诊疗详情】中进行设置");
                    return
                }
                if(!this.isCheck){
                    if(it.is_check) this.$Message.info('依据已存在！');
                    else{
                        this.activeItem1 = t;
                        this.activeItem2 = it;
                        this.index1 = i
            ...
            addSuccess(t, it, i, l, r, bool){
                ...
                if(this.type == 2 || this.type == 3){
                    ...
                    $CaseSuccessfulOperation.$emit('CaseSuccessfulOperation', this.element_id, it.id)
                } 
            },
            ...
            Interrogation(t, it, i, l, bool){
                ...
                d.case_diagnose_id = d.case_diagnoses[0].case_diagnose_id
                d.type = 1
                d.weight = 5
                d.source_type = 1
                d.relation_id = d.talk_id
                if (!it.is_check ) {
                    PostData('/v1/case/addBasis', d).then((r) => {
                        if(r.res_code == 1) this.addSuccess(t, it, i, l, r, bool)
                    })
                }else{
                    this.$Message.info('依据已存在！');
                }
            },
            setToggData(t){
                ...
                let d = {
                 ...
                 return this.tabs == 4 ? {type: t.type == 'treat' ? 'nursing' : t.type,...this.case_id, score_type: 1,} : this.tabs == 1 ? {...d,...d1} : {...d,...d2}
            },
            togg(t, i, bool = false){
                let d = this.setToggData(t)
                let url = ""
                if (this.tabs == 1) {
                    url = "/v1/inquire/getInquireListByscore"
                }else if(this.tabs == 2){
                    url = "/v1/physical/getPhysicalListByscore"
                }else{
                    url = "/v1/assistant/getAssistantListByscore"
                }
                if(!t.show){
                    PostData(url, {...d, frequency_id: this.diagnosisId,category_id:t.id}).then((r) => {
                        let da = t;
                        da.show = bool ? bool : !t.show;
                        r.data.list.forEach(element => {
                            element.third_name = element.name
                        });
