<template>
    <div class="operation-main" :style="{height: '594px'}">
       ...
       <div v-if="list&&list.length>0&&type<5" class="collapse">
                    ...
                    <div class="operation-content" v-if="t.chilrdren" @click="edit(t,i,it,l, 'supported')" v-for="(it, l) in t.chilrdren" :key="l" :class="actvieClass(it)">
                       <div class="content-detail">
                            <div class="content-left">
                                <div class="ask">
                                    <div class="grey-regular4 topic">{{it.title}}</div>
                                </div>
                            </div>
                            <div class="diagnosis-weight"><RateStar @setStar="setWeight2(t, i, it, l, $event, true)" :weight="it.weight"/></div>
                            <div v-if="!isCheck" @click.stop="deletes(t,it,i)" class="deletes"><Icon :size="20" type="ios-trash" /></div>
                       </div>
                       <div class="term-item">
       ...
       <div class="operation-content" @click="deductionEdit(it,l)" v-for="(it, l) in list" :key="l + 200" :class="activeId == (l + 1) ? 'act-content' : 'de-content'">
            <div class="content-detail">
                <div class="content-left">
                    <div class="ask">
                        <div class="grey-regular4 topic">{{it[item1]}}</div>
                ...
                <div v-if="!isCheck" @click.stop="deletes(null,it,l)" class="deletes"><Icon :size="20" type="ios-trash" /></div>
            </div>
            <div class="term-item">
                <div :class="'term-type'+it.item_type" v-if="isTerm()&&it.item_type">
                {{types[it.item_type - 1]}} <span v-if="type==2&&isTerm()">（{{it.use_count}}）</span>
    ...
    export default {
        ...
        methods: {
            getDiagnoseFrequency(){
                PostData('/v1/case/getExamineFrequency', this.case_id).then((r) => {
                    if(r.res_code == 1){
                        let d = []
            ...
            changeWeight(url, d, t, weight, i, fn){
            ...
            setWeight1(t, weight, i){
                let d = { case_diagnose_id: t.id, weight}
                this.changeWeight('v1/case/modifyDiagnose', d, t, weight, i)
            },
            deductionSetWeight(it, l, weight){
                let type = this.type < 4 ? 'basis' : it.type == 'treat' ? 'nursing' : it.type
                let d = {
                    case_diagnose_id: it.id,
                    basis_id: it.id,
                    weight,
                    type: 'deduction',
                    basis_type: type
                }
                this.changeWeight('v1/case/modifyDiagnose', d, it, weight, l, () => {
                    $CaseScoreWeightDeduction.$emit('CaseScoreWeightDeduction', weight)
                })
            },
            ...
            deductionEdit(it, l){
                ...
                let arr = ["operation", "medicine", "treat", "other"]
                this.activeId = l + 1
                this.$emit('changeState', false)
                if(it&&it.type&&arr.includes(it.type)){
                    this.$emit('changeShow', true)
                    $CaseEdit.$emit('CaseEdit', null, it.content, 'deduction')
                }else {
                    this.$emit('changeShow', false)
                    $CaseEdit.$emit('CaseEdit', null, it, 'deduction')
                }
                // 同步扣分项星星数
                $CaseScoreWeightDeduction.$emit('CaseScoreWeightDeduction', it.weight)
            },
            edit(t,i,it,l,type){
                setTimeout(() => {
                    this.supports = type
            ...
            deletes(t, it, i){
                console.log(t,it,i);
                let obj = {'talk' : "remove_talk" ,'physical' : "remove_physical",'assist' : "remove_assist",
                    operation: "remove_operation" , medicine: "remove_medicine", treat: "remove_nursing", "other": 'remove_other'}
                let remove_type = obj[it.type]
                let d = {
                    // id: it.id || it.id,
                    // ...this.case_id,
                    // remove_type,
                    // question_index: it.content.question_index || it.question_index,
                    case_diagnose_basis_id:it.id
                }
                // if(this.type == 4 || this.type == 5) d.type = it.type == 'treat' ? 'nursing' : it.type
                this.$Modal.confirm({
                    title: '你确认要删除吗？',
                    content: '<p>删除后列表里的所选数据将被清空。</p>',
                    onOk: () => {
                        PostData('/v1/case/removeDiagnoseBasis', d).then((r) => {
                            if(r&&r.res_code == 1) {
                              if(this.type == 4) {
                                   --t.count
            ...
            togg4(t, i, bool, idx){
                let d ={type: t.type == 'treat' ? 'nursing' : t.type,...this.case_id, score_type: 2, frequency_id: this.sessionStorage.getItem("FrequencyId") ? sessionStorage.getItem("FrequencyId") : this.diagnosisId}
                PostData('v1/treatment/getTreatmentListByScore',d).then((r) => {
                    let da = t;
                    da.show = bool ? bool : !t.show;
                    if (t.type == "medicine") {
                        da.chilrdren = r.data.medicine.list;
                    }else if(t.type == "treat"){
                        da.chilrdren = r.data.nursing.list;
                    }else if(t.type == "operation"){
                        r.data.operation.list.forEach(element => {
                            element.name = element.operation
                        });
                        da.chilrdren = r.data.operation.list;
                    }else{
                        da.chilrdren = r.data.other.list;
                    }
                    this.list.splice(i,1,da)
                    this.loading = false
                    if(idx == 'inits'&&da.chilrdren.length>0){
                        this.edit(t, i, da.chilrdren[0], 0, '')
                    }
                })
            },
            hide(d){
                ...
                if (t) {
                  if(!t.show || bool){
                    if(this.type && this.type < 4) {
                        ...
                        if(this.type == 4) this.togg4(t, i, bool, idx)
                    }else {
            ...
            getList(bool = false, idx){
                ...
                let d = this.type == 4 ? {score_type: 2, frequency_id: sessionStorage.getItem("FrequencyId") ? sessionStorage.getItem("FrequencyId") : this.diagnosisId} : {}
                let url = "/v1/case/getDiagnoseByScore"
                if (this.type == 4) {
                    url = "v1/treatment/getTreatmentListByScore"
                }
                PostData(url,{...d, ...this.case_id, tab_type}).then((r) => {
                    if(r&&r.data){
                        if (this.type == 1) {
                            this.list = r.data.principal.diagnose_list;
                        }else if(this.type == 2){
                            this.list = r.data.secondary.diagnose_list;   
                        }else if(this.type == 3){
                            this.list = r.data.differential.diagnose_list;
                        }else{
                            this.list = [
                                {
                        ...
                        this.list.map((t, i) => {
                            t.show = false
                            t.chilrdren = t.basis.supporting_basis
                            t.chilrdren2 = t.basis.non_supporting_basis
                        ...
                        if(this.type == 4) {
                            ...
                            this.togg(this.list[index], index, false , 'inits')
                        }
                        if(!bool){
                            if(this.type == 5) {
                                this.$emit('changeState', true)
            ...
            getScore(){
                PostData('/v1/case/getDiagnoseScore',this.case_id).then((r) => {
                    console.log(r.data.list[1]);
                    
                    let d1 = r.data.list[0] ? r.data.list[0].score : 0
                    let d2 = r.data.list[1] ? r.data.list[1].score : 0
                    let d3 = r.data.list[2] ? r.data.list[2].score : 0
                    if(r.data){
                        if(this.type == 1) {
                            this.score1 = d1
                            this.score2 = d2
