...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            saveIdentification(){
                let identify_elements = this.getIdentification()
                if(identify_elements.length>0){
                    PostData('case/saveIdentifyElement', {case_id: + sessionStorage.getItem('CASEID'), identify_elements }).then(r => {
                        if(r.res_code) sessionStorage.setItem('IdentifyElement', JSON.stringify(identify_elements))
                    })
                }else sessionStorage.setItem('IdentifyElement', JSON.stringify(identify_elements))
            },
            handleNext(save) {
            ...
            preservation(index){
                if(index == 4) {
                     this.outline.outline = JSON.stringify(this.$refs.outline.graph.save())
                     PostData('case/saveCaseOutline', {...{elements: this.setElements()}, ...this.outline}).then(res => {
                        if(res.res_code == 1) this.$Message.success('保存成功')
                     })
                }
            },
            nodeContextmenu(evt) {
