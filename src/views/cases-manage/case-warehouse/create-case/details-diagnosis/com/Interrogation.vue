        ...
        methods: {
            setOutlinesData(){
                PostData('case/getCaseDetailAndOutlines', {id: parseInt(sessionStorage.getItem('CASEID'))}).then(res => {
                    if(res.res_code == 1) {
                        this.$refs.InterrogationOutline.data = JSON.parse(res.data.outlines[0].outline)
                        this.$refs.InterrogationOutline.graph.read(JSON.parse(res.data.outlines[0].outline))
