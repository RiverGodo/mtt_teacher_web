<template>
    <div class="main">
        ...
        <Modal v-model="outlineModal" :footer-hide="true" title="查看提纲" width="1100" @on-cancel="modalCancel">
            <outlines ref="PhysicalExamOutlines" :dragcanvas="true" v-if="outlineModal" style="height: 700px;"/>
        </Modal>
    </div>
</template>
        ...
        methods: {
            setOutlinesData(){
                PostData('case/getCaseDetailAndOutlines', {id: parseInt(sessionStorage.getItem('CASEID'))}).then(res => {
                    if(res.res_code == 1) {
                        this.$refs.PhysicalExamOutlines.data = JSON.parse(res.data.outlines[0].outline)
                        this.$refs.PhysicalExamOutlines.graph.read(JSON.parse(res.data.outlines[0].outline))
                        this.$refs.PhysicalExamOutlines.updateCanvasSize()
                    }
                })
            },
            tabs(name){
