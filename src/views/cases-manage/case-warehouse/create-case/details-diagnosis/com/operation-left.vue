        ...
        <div class="details" v-show="t.show">
          ...
          <div v-if="tabindex== 1">
            ...
            <div
              ...
              :key="l"
              :class="it.id === activeId ? 'act-content' : 'de-content'"
            >
          ...
          <div v-if="tabindex== 2">
            <div
              class="content"
              v-if="t.chilrdren && it.item_type > 1"
              @click="edit(t,i,it,l)"
              v-for="(it, l) in t.chilrdren"
              :key="l"
              :class="it.id === activeId ? 'act-content' : 'de-content'"
  ...
  methods: {
    ...
    getListUrl() {
      if (this.isTerm()) {
        return [
          "case/getTalkTree",
          "case/getPhysicalFirstItem",
          "case/getAssistFirstItem",
          "case/getTreatDetailList",
        ][this.type - 1];
    ...
    getTalkUrl() {
      ...
      if (this.type == 2) {
        return this.isTerm()
          ? "case/getPhysicalResultByFirstId"
          : "case/getPhysicalResultByElement";
      }
      if (this.type == 3) {
        return this.isTerm()
          ? "case/getAssistResultByFirstId"
          : "case/getAssistResultByElement";
      }
      if (this.type == 4) {
    ...
    delteUrl() {
      return [
        "case/removeTalkBasis",
        "case/removeBasis",
        "case/removeAssistBasis",
        "case/removeTreat",
      ][this.type - 1];
    },
    deletes(t, it, i) {
    ...
    togg(t, i, bool = false) {
      ...
      if (!t.show || bool) {
        PostData(this.getTalkUrl(), {
          ...
          if(this.tabindex == 1){
            da.count = r.data.length
          } else if(this.tabindex == 2){
            // 过滤 
            da.count = r.data.filter(item => item.item_type == 2 || item.item_type == 3).length
          }
