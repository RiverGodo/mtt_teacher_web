<template>
    <div class="main">
        <div class="tabs-view tabs-content">
            <Tabs @on-click="tabs" v-model="casePane" type="line" size="small" class="tabs">
                <TabPane name="1" :label="setLabel">
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 1" :tabindex="casePane" :type="page" />
                </TabPane>
                <TabPane name="2" label="干扰项/扣分项">
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 2" :tabindex="casePane" :typePane="typePane" :type="page" />
                </TabPane>
                <TabPane name="3" label="解读分析">
                    <unscramble :diagnosisId="diagnosisId" v-if="casePane == 3" :type="page" :tabindex="casePane" />
                </TabPane>
            </Tabs>
        </div>
        ...
        <Modal v-model="outlineModal" :footer-hide="true" title="查看提纲" width="1100" @on-cancel="modalCancel">
            <outlines ref="SupplementaryExamOutlines" :dragcanvas="true" v-if="outlineModal" style="height: 700px;"/>
        </Modal>
    </div>
</template>
        ...
        methods: {
            setOutlinesData(){
                PostData('case/getCaseDetailAndOutlines', {id: parseInt(sessionStorage.getItem('CASEID'))}).then(res => {
                    if(res.res_code == 1) {
                        this.$refs.SupplementaryExamOutlines.data = JSON.parse(res.data.outlines[0].outline)
                        this.$refs.SupplementaryExamOutlines.graph.read(JSON.parse(res.data.outlines[0].outline))
                        this.$refs.SupplementaryExamOutlines.updateCanvasSize()
                    }
                })
            },
            tabs(name){
