        ...
        <div class="tabs-view tabs-content">
            <Tabs @on-click="tabs" v-model="casePane" type="line" size="small" class="tabs">
                <TabPane name="1" :label="setLabel">
                   ...
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 1" :type="page" :tabindex="casePane" />
                </TabPane>
                <TabPane name="2" label="干扰项/扣分项">
                   <div v-if="types" @click="selType" class="patient-disposition"><Icon type="ios-settings" :size="20" />  {{types}}</div>
                   <OperationLeft :diagnosisId="diagnosisId" v-if="casePane == 2" :typePane="typePane" :tabindex="casePane" :type="page" />
                </TabPane>
        ...
        methods: {
            setOutlinesData(){
                PostData('case/getCaseDetailAndOutlines', {id: parseInt(sessionStorage.getItem('CASEID'))}).then(res => {
                    if(res.res_code == 1) {
                        this.$refs.DiagnosisOutlines.data = JSON.parse(res.data.outlines[0].outline)
                        this.$refs.DiagnosisOutlines.graph.read(JSON.parse(res.data.outlines[0].outline))
