...
export default {
  ...
  methods: {
    ...
    getDiagnoseFrequency() {
      PostData("case/getDiagnoseFrequency", { case_id: this.case_id }).then(
        (r) => {
          if (r.res_code == 1) {
            let d = [];
            if (r.data.length > 0) {
    ...
    add() {
      PostData("case/addDiagnoseFrequency", { case_id: this.case_id }).then(
        (r) => {
          if (r.res_code == 1) {
...
content.scrollIntoView()
    ...
    // 获取病例所有解读
    getData(len, save) {
      PostData("case/getCaseUnscramble", { case_id: this.case_id }).then(
        (r) => {
          if (r && r.res_code == 1) {
    ...
    async checkScenceDesc() {
      ...
      await PostData("case/getScenceDesc", { id: this.diagnosisId }).then(
        (res) => {
          if (res.res_code == 1) {
    ...
    async handleNext(save) {
      ...
      let da = Array.isArray(d.force_unscramble_content)
        ? d.force_unscramble_content
        : JSON.parse(d.force_unscramble_content);
