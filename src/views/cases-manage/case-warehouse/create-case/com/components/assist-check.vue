    ...
    <div class="tabs-view tabs-content">
      ...
      >
        ...
        <TabPane :label="tabLabels[2].label" :name="tabLabels[2].name">
          <unscramble :diagnosisId="diagnosisId" :type="'3'" />
        </TabPane>
      </Tabs>
    </div>
    <div class="content let-content">
      <edit-assistcheck
        ref="refEditAssistcheck"
        :diagnosisType="Number(activeTab)"
        :group="currentSelGroup"
        :assistcheckItemData="currentSelItemData"
        @refreshData="refreshCurrentGroupList"
      />
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getInterrogationPenaltiesList() {
      ...
      postData("case/getAssistFirstItem", params).then((res) => {
        if (res.res_code === 1) {
    ...
    changeCurrentEditItem(val) {
      ...
      this.$nextTick(() => {
        this.$refs.refEditAssistcheck.initEditItemData();
      });
    },
    // 更新组列表数据
    // type 1 得分项 2 干扰项/扣分
    refreshCurrentGroupList(val) {
      ...
      this.$refs.refEditAssistcheck.clear("clearBtn");
    },
    // 给当前组添加项
    addCurrentAiagnosisGroupItem() {
      this.$refs.refEditAssistcheck.init();
    },
  },
};
</script>
