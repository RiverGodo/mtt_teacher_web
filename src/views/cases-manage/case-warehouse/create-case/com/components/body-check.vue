  ...
  <div class="bodyCheckWrapper main">
    ...
    <div class="tabs-view tabs-content">
      ...
      >
        ...
        <TabPane :label="tabLabels[2].label" :name="tabLabels[2].name">
          <unscramble :diagnosisId="diagnosisId" :type="'2'" />
        </TabPane>
      </Tabs>
    </div>
    <div class="content let-content">
      <edit-bodycheck
        ...
        :bodycheckItemData="currentSelItemData"
        @refreshData="refreshCurrentGroupList"
      />
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getInterrogationPenaltiesList() {
      ...
      postData("case/getPhysicalFirstItem", params).then((res) => {
        if (res.res_code === 1) {
    ...
    addCurrentAiagnosisGroupItem() {
      this.$refs.editBodycheck.init();
    },
    // 传递当前项的数据到编辑页面
    changeCurrentEditItem(val){
      this.currentSelItemData = val
      this.$nextTick(()=>{
        this.$refs.editBodycheck.initEditItemData()
      })
    },
    // 更新组列表数据
    // type 1 得分项 2 干扰项/扣分
    refreshCurrentGroupList(val) {
      ...
      this.$refs.editBodycheck.clear('clearBtn');
    },
  },
};
</script>
