<template>
  <div class="sceneDescription">
    ...
    <Button v-if="!isCheck" class="scene-btns" @click="Preservation"
      >保存</Button
    >
    <Modal class="scene-modals" footer-hide v-model="modal1" width="506">
      ...
      <div v-if="panes == 1" class="content">
        <p>
          1、门诊室进来一位中年男子，黑胖胖的脸上留着短须，身形消瘦，精神萎靡。
        </p>
        <Divider />
        <p>
          2、一位年轻的女士在男士的陪同下，手捂着肚子，表情十分痛苦，缓慢的走进门诊室。
        </p>
      </div>
      <div v-if="panes == 2" class="content">
        ...
        <p>
          术后5天，患者精神状态依然不佳，面色发黄，病情未见好转迹象，仍不可下床行走。
        </p>
      </div>
    </Modal>
  </div>
