<template>
    <div style="height: 100%; overflow-y: auto">
        ...
        <div style="height: calc(100% - 50px);position: relative">
            <!--G6模版-->
            <div id="mountNode" class="mountNode" style="display:inline-block"></div>
        ...
        methods: {
            ...
            addNode(type) {
                ...
                this.node = this.graph.addItem('node', {
                    ...
                    label: '添加内容',
                    type: type,
                    children: [{id: String(new Date().getTime()), label: '添\n加\n内\n容'}]
                });
            ...
            editNode() {
                ...
                console.log(this.evt.target.get('childIndex'))
                ...
                if(this.evt.item._cfg.model.case_result) {
                    ...
                    this.$refs.case_input.$el.style.left = `${document.getElementById("mountNode").offsetLeft+point.x}px`;
                    this.$refs.case_input.$el.style.top = `${point.y}px`;
                    this.$refs.case_input.focus()
                    this.$refs.case_menu.style.left = '-150px';
                }
            ...
            editNodeGather() {
                ...
                // model.children[this.evt.target.get('childIndex')].label = '3456446'
                // console.log(this.evt.target);
                // model.children[this.gatherNum].style = {}
                this.evt.target.attrs.height = 500+'px'
                const point = this.graph.getCanvasByPoint(model.x - model.size[0]/2 , model.y - model.size[1]/2 + 10);
                this.$refs.case_input.$el.style.left = `${ document.getElementById("mountNode").offsetLeft+point.x + 50 * this.gatherNum - 30}px`;
                this.$refs.case_input.$el.style.top = `${point.y}px`;
                this.$refs.case_input.focus()
                this.$refs.case_menu.style.left = '-150px';
                // this.updateGraph()
            ...
            inputBlur() {
                if(this.evt.item._cfg.type == 'edge') this.evt.item._cfg.model.text = this.input_data
                    ...
                    const label_data = model.children[this.gatherNum].label
                    // 格式化输入
                    // const pattern = new RegExp("[\u4E00-\u9FA5]+");
                    // this.input_data = this.input_data.split('').forEach((letter, i) => {
                    //     // if (pattern.test(letter)) {
                    //         letter+= '\n'
                    //     // }
                    // });
                    let input_txt = ''
                    let is_not_hans = false
                    this.input_data.split('').forEach(letter => {
                    //     if (pattern.test(letter)){
                    //         if(is_not_hans){
                    //             input_txt+=`\n${letter}\n`
                    //         }else{
                    ...
                    if(label_data == input_txt || !input_txt){
                        model.children[this.gatherNum].label = label_data
                    }else{
                        model.children[this.gatherNum].label = input_txt
                    }
                    model.children[this.gatherNum].height = 500
            ...
            initGraph() {
                let that = this
                // 配置绘画参数
                let border_width_default = 2, // 边框宽
                ...
                gather_outer_rect_width_default = 120,// 结合节点收起默认宽
                rect_single_width_default = 120, // 单个矩形节点默认宽
                rect_single_height_default = 36, // 单个矩形节点默认高
                rect_child_width_default = 36, // 集合节点子矩形默认宽
                rect_child_height_default = 100,// 集合节点子矩形默认高
                
                gather_close_circle_fill_default = '#4098FF'// 收起节点圆形背景颜色
                    ...
                    draw(cfg, group) {
                        ...
                        const rect = group.addShape('rect', {
                            attrs: {
                                x: -width / 2,
                                y: -height / 2,
                                stroke: cfg.stroke || stroke_color_default,
                                width: width,
                                height: height,
                                fill: cfg.fill || fill_color_default,
                                radius: border_radius_default,
                                lineWidth: border_width_default,
                            },
                            className: 'rectData'
                        })
                        if (cfg.label) {
                            group.addShape('text', {
                                attrs: {
                                    ...
                                    x: 0,
                                    y: 0,
                                    fill: (cfg.label === '添加内容' || cfg.label === '请输入病种名称') ? color_placeholder_default : '#333',
                        ...
                        if (cfg.init) {
                            group.addShape('rect', {
                                attrs: {
                                    ...
                                    width: 5,
                                    height: height,
                                    // fill: cfg.degree == 1 ? that.degree_list[0].color : cfg.degree == 2 ? '#4098ff' : '#FD6C1E',
                                    fill: cfg.degree ? that.degree_list[cfg.degree-1].color : fill_color_default,
                                    // stroke: cfg.degree == 1 ? '#7ED321' : cfg.degree == 2 ? '#4098ff' : '#FD6C1E',
                                    stroke: cfg.degree ? that.degree_list[cfg.degree-1].color : stroke_color_default,
                                    radius: [5, 0, 0, 5],
                                }
                            })
                        }
                ...
                G6.registerNode('gather', {
                    draw(cfg, group) {
                        cfg.size = [cfg.children.length * rect_child_width_default , rect_child_width_default];
                        // 子矩形宽高
                        let size = [cfg.children.length * rect_child_width_default , rect_child_height_default];
                        let width = size[0];
                        let height = size[1];
                        let rect = {}
                        // 折叠
                        if (cfg.pack) {
                            rect = group.addShape('rect', {
                                attrs: {
                                    ...
                                    y: -height / 2,
                                    stroke: '#4098ff',
                                    width: gather_outer_rect_width_default,
                                    height: height + 20,
                                    fill: cfg.fill || '#fff',
                                    radius: border_radius_default,
                                    lineWidth: border_width_default,
                            ...
                            // 外层矩形
                            let outer_rect_height =  Math.max(...cfg.children.map(x => x.label.split('\n').length)) * 20;
                            ...
                            rect = group.addShape('rect', {
                                attrs: {
                                    ...
                                    fill: cfg.fill || fill_color_default,
                                    opacity: 0.3,
                                    // width: cfg.add ? cfg.children.length * 50 + 50 : cfg.children.length * 50,
                                    width: cfg.add ? cfg.children.length * (rect_child_width_default + 10) + (rect_child_width_default+10) : cfg.children.length * (rect_child_width_default + 10),
                                    height: height + 40,
                                    radius: 5,
                            ...
                            cfg.children.forEach((item, index) => {
                                ...
                                let labelLen = item.label.split('\n').length
                                height = (labelLen+1 )* 20


                                let child = group.addShape('rect', {
                                    attrs: {
                                        x: (rect_child_width_default + 10) * index - width / 2 + 5,
                                        y: -10,
                                        stroke: stroke_color_default,
                                        width: rect_child_width_default,
                                        height: height,
                                        fill: fill_color_default,
                                        radius: border_radius_default,
                                        lineWidth: border_width_default,
                                ...
                                group.addShape('text', {
                                    attrs: {
                                        text: item.label,
                                        // x: 50 * index - width / 2,
                                        x: (rect_child_width_default + 10) * index - width / 2 + (rect_child_width_default + 10)/2,
                                        y: height / 2,
                                        fill: item.label === '添\n加\n内\n容' ? '#ABB7C5' : '#333',
                                        fontSize: 14,
                            ...
                            if(cfg.add) {
                                ...
                                group.addShape('rect', {
                                    attrs: {
                                        x: cfg.children.length * (rect_child_width_default + 10) - width / 2 + 10,
                                        y: -5,
                                        width: 30,
                                        height: 30,
                                        stroke: '#ABB7C5',
                                ...
                                group.addShape('text', {
                                    attrs: {
                                        x: cfg.children.length * (rect_child_width_default + 10) - width / 2 + (rect_child_width_default + 10)/2,
                                        y: 5,
                                        fontSize: 40,
                                        textAlign: 'center',
                                        textBaseline: 'middle',
                        ...
                        group.addShape('circle', {
                            attrs: {
                                x: cfg.pack ? 0 : cfg.children.length * (rect_child_width_default+10)/2 - width / 2,
                                y: -height / 2 - 5,
                                r: 10,
                                fill: gather_close_circle_fill_default,
                                stroke: gather_close_circle_fill_default,
                                cursor: 'pointer',
                            },
                            className: 'packUp'
                        })
                ...
                G6.registerNode('opinion', {
                    draw(cfg, group) {
                        ...
                        if(cfg.label!='添加内容'){
                            width = (cfg.label.split('').length + 2) *20
                ...
                this.graph = new G6.Graph({
                    ...
                    defaultNode: {
                        type: 'rect',
                        // size: [120, 40],
                        size: [rect_single_width_default, rect_single_height_default],
                        fill: fill_color_default,
                        stroke: stroke_color_default,
                        anchorPoints: [
                            [0.5, 0], [0.5, 1]
                ...
                this.graph.on('node:contextmenu', evt => {
                    ...
                    this.$refs.case_menu.style.left = document.getElementById("mountNode").offsetLeft+point.x+`px`;
            ...
            updateGraph() {
                ...
                // console.log(this.data, 'outlineaaa');
                this.graph.clear()
                this.graph.read(this.data)
            ...
            updateCanvasSize(){
                let max__node_h = Math.max(...this.data.nodes.map(x => x.y));
                let max__node_w = Math.max(...this.data.nodes.map(x => x.x));
                let graph_h = max__node_h > 800 ? max__node_h+500 : 1100
                let graph_w = max__node_w > 800 ? max__node_w+300 : 1100
                this.graph.changeSize(graph_w,graph_h);
            }
        },
    }
</script>
