<template>
  <Modal v-model="show" title="查看" :width="(type==3 || type==5 || type==6) ? 1000 : 620" @on-cancel="closeModal"  :mask-closable=false :footer-hide="true" >
      ...
      <div v-if="type==5 || type==6" :class="detailData&&detailData.type=='ExaminationReport' ? 'examination-report':''">
        ...
        <div class="assistant" v-if="(detailData.type=='ImageData' || detailData.type=='Word')&&detailData.detail.length>0">
            <div  class="image-data">
                <!-- assistant.assistant_details[0].report -->
                <iframe  class="image-data-iframe" :src='officeappsUrl+detailData.detail[0].report' width='100%' height='100%' frameborder='1' />
            </div>
        </div>
        <div class="assistant-item" style="text-align:center;" v-if="detailData.type=='Picture'&&detailData.detail.length>0">
            <img width="90%" style="display:inline-block;margin-top:20px;" v-for="(t,i) in detailData.detail" :key="i" :src="mediaUrl+t.report" alt="检查结果图片" />
        </div>
        <div class="assistant-item" style="text-align:center;" v-if="detailData.type=='Video'&&detailData.detail.length>0">
            <video v-for="(t,i) in detailData.detail" :key="i" :src="mediaUrl+t.report" alt="检查结果视频"></video>
        </div>
        <div class="assistant-item" style="text-align:center;" v-if="detailData.type=='Audio'&&detailData.detail.length>0">
            <audio v-for="(t,i) in detailData.detail" :key="i" :src="mediaUrl+t.report" alt="检查结果音频"></audio>
        </div>
      </div>
      </div>
  </Modal>
