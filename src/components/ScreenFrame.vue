<template>
    <div class="screen-box" :style="typeArr.includes('backgroundColor') ? backgroundColor:''">
        <div :class="typeArr.includes('searchDropDown') ? 'screenSearchDrop': 'screen'" :style="typeArr.includes('backgroundColor') ? backgroundColor:''">
            ...
            <!-- <Select v-if="typeArr.includes('select2_3')" v-model="valueSelect2_3" @on-change="selectChange2_3" class="select-list" :placeholder="select2_3Placeholder">
                <Option v-for="item in select2_3" :value="item.id" :key="item.id"></Option>
            </Select> -->
        ...
        <!-- 导入/导出 -->
        <div class="right-btn">
             <Button v-if="typeArr.includes('button1')&&btnName1 == '导入/导出'" :class="visibilityBtn"  @click="handleClick1">
                <Icon size="18" type="ios-share" /> {{btnName1}}
            </Button>
            <Button v-else-if="typeArr.includes('button1')" :class="visibilityBtn"  @click="handleClick1">{{btnName1}}</Button>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.setlectInit('select2_3','valueSelect2_3');
            ...
            if(this.typeArr.includes('isSuper')&&this.isSuper){
                // this.$config.setHandleSecond(() => {
                    postData('/v1/common/getOrganizationList').then((res) => {
                        // console.log(res.data,'res.data')
                        this.select = [{id: 'all', title: '全部机构'},...res.data]
