<template>
    <div>
        <Table id="all-tables" :highlight-row="highlightRow" @on-row-click="rowClick" :columns="columns" :data="datas" ref="tables"  @on-expand="expand"  :height="tabelHeight" @on-select="selectTables" @on-select-all="selectAll" @on-select-all-cancel="selectAllCancel" @on-select-cancel="selectCancel">
            <!-- content-html -->
            ...
            <template slot-scope="{ column, row, index }" slot="operation">
                ...
                <Switch :class="column.isShow ? '' : 'operation_btn_show'" v-if="column.isSwitch && handleBtnShow(column,row)&&!column.deletes" v-model="row[column.switchKey]" size="large" @on-change="change(row)">
                    <span slot="open">{{column.switchList[0]}}</span>
                    <span slot="close">{{column.switchList[1]}}</span>
                </Switch>
